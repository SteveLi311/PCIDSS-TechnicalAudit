...
mkdir -p report
OUTPUT_FILE="report/check_nacl_inbound.xml"
...
for vpc_id in $VPCS; do
  ...
  for subnet_id in $SUBNETS; do
    ...
    rules=$(aws ec2 describe-network-acls --region "$REGION" --network-acl-ids $nacl_id --query 'NetworkAcls[0].Entries[?Egress==`false` && CidrBlock==`0.0.0.0/0` && RuleAction==`allow`]' --output text)
